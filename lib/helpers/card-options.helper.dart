...
Future<String?> switchLegendModal(BuildContext context, String deckSlug, int currentLeaderId,
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
          void save() async {
            ...
            final response = await updateLegendDeck(deckSlug, cards[currentIndex].id);
                              ...
                              child: SizedBox(
                                ...
                                child: RawMaterialButton(
                                  onPressed: () async {
                                    ...
                                    // switchLegendModal(context, cards);
                                    // switchLegendModal(context, widget.card);
                                    setState(() {
                                      currentIndex = index;
                                    });
                                  },
...
Future<String?> switchChampionModal(BuildContext context, String deckSlug, int currentChampionId,
    List<CardItemView> cards, Color backgroundColor, Color foregroundColor) async {
  int currentIndex = cards.indexWhere((card) => card.id == currentChampionId);
  final response = await showModalBottomSheet(
      context: context,
      useSafeArea: true,
      isScrollControlled: true,
      builder: (BuildContext context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setState) {
          void save() async {
            if (cards[currentIndex].id == currentChampionId) {
              Navigator.pop(context);
              return;
            }

            final response = await updateChampionDeck(deckSlug, cards[currentIndex].id);
            return response.fold((l) {
              Navigator.pop(context, cards[currentIndex].thumbnail);
            }, (r) {
              Navigator.pop(context);
                          ...
                          child: Stack(children: [
                            ...
                            Positioned(
                              ...
                              child: SizedBox(
                                ...
                                child: RawMaterialButton(
                                  onPressed: () async {
                                    // _getChampions(widget.card.cardId);
                                    // final cards = await _getChampions(widget.card.cardId);
                                    // switchChampionModal(context, cards);
                                    // switchChampionModal(context, widget.card);
                                    setState(() {
                                      currentIndex = index;
                                    });
                                  },
