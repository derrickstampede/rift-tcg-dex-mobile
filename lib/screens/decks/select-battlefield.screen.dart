...
class SelectBattlefieldScreen extends ConsumerStatefulWidget {
  const SelectBattlefieldScreen({super.key});

  @override
  ConsumerState<SelectBattlefieldScreen> createState() => _SelectBattlefieldScreenState();
}

class _SelectBattlefieldScreenState extends ConsumerState<SelectBattlefieldScreen> {
  ...
  final CardSearch _cardSearch = CardSearch(
    ...
    config: CardSearchConfig(
      ...
      disableColor: const ["Red", "Green", "Blue", "Orange", "Purple", "Yellow", "No Color"],
      initialResetColor: const [],
      initialResetType: const ["Battlefield"],
                ...
                if (!search$.status.isInitializing) ...[
                  for (int i = 0; i < search$.cardBatches.length; i++) ...[
                    ...
                    if (!_isPro &&
                      ...
                      Padding(
                        key: ValueKey('ad_banner_select_battlefield_$i'),
                        padding: const EdgeInsets.symmetric(vertical: 2),
                        child: Center(child: AdBanner(key: ValueKey('ad_banner_select_battlefield_inner_$i'))),
                      ),
                  ],
